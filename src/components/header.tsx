...
export function Header({ onNavClick }: { onNavClick?: (tab: string) => void }) {
  const { user, loading: authLoading } = useAuth()
  const [isMenuOpen, setIsMenuOpen] = useState(false)

  return (
    <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-xl border-b border-white/[0.18]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2">
            <span className="text-xl font-black tracking-tight text-white">SZENE</span>
            <span className="hidden sm:inline text-[10px] text-white/55 uppercase tracking-[0.2em] font-medium mt-0.5">Mannheim</span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {[
              ...
              <button key={l.label} onClick={() => onNavClick?.(l.tab)}
        ...
        {/* Mobile nav */}
        {isMenuOpen && (
          <div className="md:hidden py-4 border-t border-white/[0.18] space-y-1">
            {[
              { label: "Tonight", tab: "tonight" },
              { label: "Events",  tab: "events" },
              { label: "Venues",  tab: "venues" },
            ].map(l => (
              <button key={l.label} onClick={() => { onNavClick?.(l.tab); setIsMenuOpen(false) }}
                className="block w-full text-left py-2 text-sm text-white/60 hover:text-white transition-colors">{l.label}</button>
            ))}
            {!authLoading && !user && (
