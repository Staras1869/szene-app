...
export function AiChat() {
  ...
  return (
    <>
      ...
      {/* Chat drawer */}
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-full sm:w-[380px] bg-white border-2 border-gray-200 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-violet-600 rounded-t-3xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-violet-200 text-xs">Your night-out concierge</p>
              </div>
            </div>
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-[340px] bg-gray-50">
            {messages.map((msg, i) => (
          ...
          {messages.length === 1 && (
            <div className="px-4 pb-2 pt-3 flex flex-wrap gap-2 bg-white border-t border-gray-100">
              {STARTERS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
